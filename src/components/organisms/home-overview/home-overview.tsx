...
import { NavigationPill } from "@/components/atoms/navigation-pill";
...
const HomeOverview: FC<HomeOverviewProps> = ({
  ...
  const recentExpenses = useRecentTransactionsBeforeDateLive(
    selectedDate,
    recentLimit,
  );
  ...
  const groupedRecentExpenses = useMemo(() => {
    ...
    const groups: {
      key: string;
      label: string;
      expenses: typeof recentExpenses;
    }[] = [];
    for (const expense of recentExpenses) {
      const d = dayjs(expense.transaction_date);
      const key = d.format("YYYY-MM-DD");
      const isYesterdayDate = isSameDay(
        d.toDate(),
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (
          entry.isIntersecting &&
          recentExpenses &&
          recentExpenses.length === recentLimit
        ) {
  ...
  return (
    <div className="py-4">
      ...
      <NavigationPill
        label={dateLabel}
        isNextDisabled={isTodaySelected}
